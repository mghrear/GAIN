...
# In Dir 
in_dir = '/Users/mghrear/data/LDMX_GNN/inclusive/root/'
# Out Dir
out_dir = '/Users/mghrear/data/LDMX_GNN/inclusive/processed/'

passName="TrackerReco" # "TrackerReco", "signal"
...
for file_name in file_names:
    ...
    SimParticles_processType = root_file['SimParticles_'+passName+'/SimParticles_'+passName+'.second.process_type_'].array()
    SimParticles_pdgID = root_file['SimParticles_'+passName+'/SimParticles_'+passName+'.second.pdg_id_'].array()
    ...
    Tagger_SimHits_pdgID = root_file['TaggerSimHits_'+passName+'/TaggerSimHits_'+passName+'.pdg_id_'].array()
    Tagger_SimHits_Edep = root_file['TaggerSimHits_'+passName+'/TaggerSimHits_'+passName+'.edep_'].array()
    Tagger_SimHits_trackID = root_file['TaggerSimHits_'+passName+'/TaggerSimHits_'+passName+'.track_id_'].array()

    Tagger_Digi_x = root_file['DigiTaggerSimHits_'+passName+'/DigiTaggerSimHits_'+passName+'.meas_x_'].array()
    Tagger_Digi_y = root_file['DigiTaggerSimHits_'+passName+'/DigiTaggerSimHits_'+passName+'.meas_y_'].array()
    Tagger_Digi_z = root_file['DigiTaggerSimHits_'+passName+'/DigiTaggerSimHits_'+passName+'.meas_z_'].array()
    Tagger_Digi_Edep = root_file['DigiTaggerSimHits_'+passName+'/DigiTaggerSimHits_'+passName+'.edep_'].array() 
    Tagger_Digi_trackID = root_file['DigiTaggerSimHits_'+passName+'/DigiTaggerSimHits_'+passName+'.track_ids_'].array()

    Tagger_Truth_ID = root_file['TaggerTruthTracks_'+passName+'/TaggerTruthTracks_'+passName+'.track_id_'].array()
    ...
    Recoil_SimHits_pdgID = root_file['RecoilSimHits_'+passName+'/RecoilSimHits_'+passName+'.pdg_id_'].array()
    Recoil_SimHits_Edep = root_file['RecoilSimHits_'+passName+'/RecoilSimHits_'+passName+'.edep_'].array()
    Recoil_SimHits_trackID = root_file['RecoilSimHits_'+passName+'/RecoilSimHits_'+passName+'.track_id_'].array()

    Recoil_Digi_x = root_file['DigiRecoilSimHits_'+passName+'/DigiRecoilSimHits_'+passName+'.meas_x_'].array()
    Recoil_Digi_y = root_file['DigiRecoilSimHits_'+passName+'/DigiRecoilSimHits_'+passName+'.meas_y_'].array()
    Recoil_Digi_z = root_file['DigiRecoilSimHits_'+passName+'/DigiRecoilSimHits_'+passName+'.meas_z_'].array()
    Recoil_Digi_Edep = root_file['DigiRecoilSimHits_'+passName+'/DigiRecoilSimHits_'+passName+'.edep_'].array() 
    Recoil_Digi_trackID = root_file['DigiRecoilSimHits_'+passName+'/DigiRecoilSimHits_'+passName+'.track_ids_'].array()

    Recoil_Truth_ID = root_file['RecoilTruthTracks_'+passName+'/RecoilTruthTracks_'+passName+'.track_id_'].array()
    Recoil_Truth_P = (1.0/root_file['RecoilTruthTracks_'+passName+'/RecoilTruthTracks_'+passName+'.perigee_pars_'].array())[:,:,4]

    Process_Type = root_file['SimParticles_'+passName+'/SimParticles_'+passName+'.second.process_type_'].array()


    rows = []
    for i in range(len(Tagger_SimHits_x)):
        rows.append({"SimParticles_first": np.array(SimParticles_first[i]),
                     "SimParticles_n_primaries": np.sum(np.array(Process_Type[i]) == 13),
